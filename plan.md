...
I would like to focus now on manually importing users from a given learn.db database backup. I want to import all users from the backup who are not edwinlock@gmail.com, michelle.gonzalezam@gmail.com, or esther.gehrke@wur.nl. I don't need to transfer the "role" field. Indeed, we should keep that field empty for now, and then user user_datastore.add_role() to make each of the users an enumerator. Write a Python script for this task. We can assume that the User model in the backup is the same as in the current flask app.